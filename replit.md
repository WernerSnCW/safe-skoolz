...
- **Training Completion Tracking:** Staff can mark training modules as read on the "Using safeskoolz" tab (/learn). 9 modules tracked: loggingIncident, assessingIncident, managingPupilPins, behaviourPoints, respondingToMessages, understandingAlerts, managingProtocols, sencoCaseload, dashboardOverview. Coordinators/head teachers can view staff completion matrix at /training-status with CSV export. Summary card on coordinator dashboard links to the full view. New table: training_completions (userId+moduleId unique constraint). Audit event: training_module_completed.
