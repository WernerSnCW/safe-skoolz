...
- New table `diagnostic_answers`: `surveyId`, `responseId` (a random UUID shared by one respondent's answers — exists **only** here, never on the submission), `questionKey`, `answer` (int) / `freeText`, `createdAt`. **No foreign key to the submission — unlinkable by design.** All rows written in one transaction.
- **Optional demographics for segmentation**: respondents may select year group and class/teacher ("if they want to"). These attach to the anonymous `responseId` (a `diagnostic_response_meta` row: `surveyId`, `responseId`, `yearGroup?`, `classOrTeacher?`), never to the email record — so results can be tracked **by year group** without weakening anonymity.
- **Small-segment suppression**: segmented breakdowns render only when the segment has ≥ 5 responses; smaller segments roll up (class → year group → whole school) so no individual's answers can be inferred from a thin slice.
